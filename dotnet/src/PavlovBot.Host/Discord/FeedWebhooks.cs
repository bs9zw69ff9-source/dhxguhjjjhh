...
/// <summary>
/// The plain-text feeds: joins, leaves and kills.
/// </summary>
/// <remarks>
...
public sealed class FeedWebhooks : IAsyncDisposable
{
    ...
    /// </summary>
    /// <remarks>
    /// A SUICIDE IS NOT AN ARROW. A self-kill has killer and killed as the same person, which
    /// rendered as "Name → Name" and reads like a parser fault every time. Pavlov files a fall
    /// or a drowning the same way with no weapon, so those two are told apart by whether a
    /// weapon was named.
    ///
    /// The headshot flag is carried but never set: Pavlov.log's verbose kill output does not
    /// have one. It stays because the rendering is right the day a source does.
    /// </remarks>
