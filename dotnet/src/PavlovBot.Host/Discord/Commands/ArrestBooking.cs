...
public sealed class ArrestBooking(ILogger<ArrestBooking> logger) : IComponentHandler
{
    ...
    private sealed record Booking(string Player, List<string> Codes, double Rate, ulong OwnerId, DateTimeOffset At);
    ...
    public async Task BeginAsync(SocketSlashCommand command, string player, double rate)
    {
        ArgumentNullException.ThrowIfNull(command);

        var booking = new Booking(player, [], rate, command.User.Id, DateTimeOffset.UtcNow);
    ...
    {
        var result = PenalCode.Book(booking.Codes, booking.Rate);
        ...
        return Theme.Warning($"Booking: {Sanitize.Code(booking.Player)}",
                ...
                $"**Jail:** {result.JailMinutes} min  •  **Bail:** ${result.Bail:N0}\n{lines}")
            .Brand()
            .Build();
    }
