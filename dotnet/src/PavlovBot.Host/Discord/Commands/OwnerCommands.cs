...
/// <summary>
/// <c>/configure</c> - the destructive owner operations, behind a typed confirmation.
/// </summary>
/// <remarks>
/// Each of these wipes something that cannot be reconstructed from anywhere else, so none
/// of them fire on the command alone: the caller has to type the exact word back. A button
/// or a boolean flag is not enough - both are one misclick, and a wiped ban list on a
/// server that has been running for a year is not recoverable from a backup nobody took.
/// </remarks>
public sealed class ConfigureCommand(
    SerializedStore store, IpTrackingService tracking, AuditLog audit, Access access, ILogger<ConfigureCommand> logger) : ISlashCommand
{
    ...
    public ApplicationCommandProperties Build() =>
        new SlashCommandBuilder()
            .WithName(Name)
            .WithDescription("Owner - Destructive maintenance operations")
            .AddOption(new SlashCommandOptionBuilder()
                .WithName("operation").WithDescription("What to wipe")
                .WithType(ApplicationCommandOptionType.String).WithRequired(true)
                .AddChoice("Clear all temporary bans", "cleartempbans")
                .AddChoice("Clear ALL bans", "clearallbans")
                .AddChoice("Strip every menu grant", "stripmenuall")
                .AddChoice("Clear every IP and account flag", "clearflags"))
            .AddOption("confirm", ApplicationCommandOptionType.String,
                "Type the operation name exactly to confirm", isRequired: true)
            .Build();
    ...
    public async Task HandleAsync(SocketSlashCommand command, CancellationToken ct)
    {
        ...
        var operation = command.Data.Options.First(o => o.Name == "operation").Value as string ?? "";
        var confirm = (command.Data.Options.First(o => o.Name == "confirm").Value as string ?? "").Trim();

        if (!string.Equals(operation, confirm, StringComparison.OrdinalIgnoreCase))
        {
            await Reply(command, Theme.Failure("Not confirmed",
                $"Type `{operation}` exactly into the confirm field. Nothing was changed.")).ConfigureAwait(false);
            return;
        }
        ...
        switch (operation)
        {
            case "cleartempbans":
            {
                ...
                affected = before.Count - kept.Count;
                await store.WriteAsync(Datasets.TempBans, kept, ct).ConfigureAwait(false);
                what = "temporary ban(s) cleared. Permanent bans were kept.";
                break;
            }

            case "clearallbans":
            {
                affected = store.Read<List<BanRecord>>(Datasets.TempBans, []).Count;
                await store.WriteAsync<List<BanRecord>>(Datasets.TempBans, [], ct).ConfigureAwait(false);
                /* The bot's record only. Native server bans are NOT lifted here - doing that
                   would be one command that unbans everyone on every server, and the
                   reconcile pass would then have nothing to re-apply. */
                what = "ban record(s) cleared from the bot. Native server bans are untouched - lift those with `/unban`.";
                break;
            }

            case "stripmenuall":
            {
                affected = store.Read(Datasets.MenuGrants, new Dictionary<string, string>(StringComparer.Ordinal)).Count;
                await store.WriteAsync(Datasets.MenuGrants, new Dictionary<string, string>(StringComparer.Ordinal), ct).ConfigureAwait(false);
                // Bindings survive. Clearing them too would let everyone re-claim as an alt,
                // which is the exact thing the binding exists to prevent.
                what = "menu grant(s) removed. Name bindings are unchanged.";
                break;
            }
        ...
        await audit.RecordAsync($"configure-{operation}", command.User.Username, "(server-wide)",
            $"{affected} affected", ct).ConfigureAwait(false);
        logger.LogWarning("DESTRUCTIVE {Operation} | affected={Affected} | by={By} | at={At}",
            operation, affected, command.User.Username, now);

        await Reply(command, Theme.Warning($"{operation} done", $"**{affected}** {what}")
