...
public sealed class StaticMap(HttpClient http, string apiKey, ILogger<StaticMap> logger)
{
    ...
    /// <summary>
    /// Total rendered bytes the per-key cache may hold before evicting the least-recently-used.
    /// A single map can be up to <see cref="MaxBytes"/>, so the cache is bounded by BYTES, not a
    /// count - a count cap could pin hundreds of MB. 16 MB covers an active server's recent IPs
    /// while keeping the ceiling flat.
    /// </summary>
    private const long CacheByteBudget = 16 * 1024 * 1024;

    private readonly object _cacheLock = new();
    private readonly Dictionary<string, LinkedListNode<Cached>> _cache = new(StringComparer.Ordinal);
    private readonly LinkedList<Cached> _lru = new();   // most-recently-used at the FRONT
    private long _cachedBytes;

    private sealed record Cached(string Key, byte[] Bytes);

    /// <summary>
    /// Render for a caller-supplied key (an IP), reusing a previous render for that key instead
    /// of spending another Geoapify call.
    /// </summary>
    /// <remarks>
    /// SO A BUSY CONNECT FEED COSTS ONE RENDER PER IP, not one per connection. An IP's location
    /// does not move, so a cached map stays correct; the cache is LRU-evicted under
    /// <see cref="CacheByteBudget"/> so it cannot grow without bound. A rare double-render can
    /// happen if two connections from a brand-new IP race the first miss - not worth coalescing
    /// for a map, and steady-state repeats are fully served from the cache.
    /// </remarks>
    public async Task<byte[]?> RenderForAsync(string cacheKey, double latitude, double longitude, CancellationToken ct)
    {
        if (!string.IsNullOrEmpty(cacheKey))
        {
            lock (_cacheLock)
            {
                ...
                {
                    _lru.Remove(node);
                    _lru.AddFirst(node);   // touch: most-recently-used
                    return node.Value.Bytes;
                }
            }
        }

        var bytes = await RenderAsync(latitude, longitude, ct).ConfigureAwait(false);
        if (bytes is null || string.IsNullOrEmpty(cacheKey)) return bytes;

        lock (_cacheLock)
        {
            // Another caller may have populated it while this one was rendering; keep the first.
            if (!_cache.ContainsKey(cacheKey))
            {
                var node = _lru.AddFirst(new Cached(cacheKey, bytes));
                _cache[cacheKey] = node;
                _cachedBytes += bytes.Length;

                while (_cachedBytes > CacheByteBudget && _lru.Last is { } oldest)
                {
                    _lru.RemoveLast();
