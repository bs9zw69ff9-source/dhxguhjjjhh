...
public class StaticMapTests
{
    ...
    [Fact]
    public async Task ASecondRenderForTheSameKeyIsServedFromCacheWithNoApiCall()
    {
        // The whole point of RenderForAsync: the map for an IP is drawn once and reused, so a
        // busy connect feed spends one Geoapify call per address, not one per connection.
        var handler = new MapHandler(HttpStatusCode.OK, Png);
        var map = Map(handler);

        var first = await map.RenderForAsync("203.0.113.9", 43.65, -79.38, CancellationToken.None);
        var second = await map.RenderForAsync("203.0.113.9", 43.65, -79.38, CancellationToken.None);

        Assert.Equal(Png, first);
        Assert.Equal(Png, second);
        Assert.Equal(1, handler.Calls);   // second connection cost no quota
    }

    [Fact]
    public async Task DifferentKeysEachRender()
    {
        // The cache keys on the caller's key (the IP), so two different addresses are two
        // renders even at identical coordinates - no false sharing between IPs.
        var handler = new MapHandler(HttpStatusCode.OK, Png);
        var map = Map(handler);

        await map.RenderForAsync("203.0.113.9", 43.65, -79.38, CancellationToken.None);
        await map.RenderForAsync("198.51.100.7", 43.65, -79.38, CancellationToken.None);

        Assert.Equal(2, handler.Calls);
    }

    [Fact]
    public async Task AFailedRenderIsNotCachedSoALaterConnectionRetries()
    {
        // A map-service blip must not poison the cache with a null: the next connection from
        // that IP should try again rather than being stuck with no picture forever.
        var handler = new CountingSequenceHandler(
            new HttpResponseMessage(HttpStatusCode.TooManyRequests) { Content = new ByteArrayContent([]) },
            Ok());
        var map = new StaticMap(new HttpClient(handler), "secret-key", NullLogger<StaticMap>.Instance);

        var failed = await map.RenderForAsync("203.0.113.9", 43.65, -79.38, CancellationToken.None);
        var retried = await map.RenderForAsync("203.0.113.9", 43.65, -79.38, CancellationToken.None);

        Assert.Null(failed);
        Assert.Equal(Png, retried);
        Assert.Equal(2, handler.Calls);   // the failure was retried, not cached
    }
    ...
    /// <summary>Serves a queued sequence of responses, one per call.</summary>
    private sealed class CountingSequenceHandler(params HttpResponseMessage[] responses) : HttpMessageHandler
    {
        private int _index;
        public int Calls { get; private set; }
