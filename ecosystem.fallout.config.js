/* pm2 config for the FALLOUT BOT, which is the bot.
 *
 * IT WAS DELETED WITH THE SECOND-BOT MACHINERY AND SHOULD NOT HAVE BEEN. The
 * clone arrangement is gone - there is no police bot to be second to - but this
 * is the config of the process that SURVIVED it, running under pm2 as
 * pavlov-bot-fallout with its .env and data in its own directory. Removing it
 * left `pm2 restart ecosystem.fallout.config.js` with nothing to read, which is
 * the live bot unable to be restarted at all.
 *
 *
 ...
 *
 * ONE TOKEN PER PROCESS. Two processes on one token answer every command twice,
 * and the check for that is the token in each .env.
 */
